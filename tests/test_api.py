...
from nba_projection_bot.schemas import AskResponse

client = TestClient(api.app)


@pytest.mark.skipif(
    not (api.STATIC_DIR / "index.html").is_file(),
    reason="frontend build output not present (run `npm run build` in frontend/)",
)
...
def test_ask_response_projections_default_empty():
    resp = AskResponse(answer="hi", conversation_id=1)
...
def test_ask_response_carries_projections():
    ...
    resp = AskResponse(answer="hi", conversation_id=1, projections=proj)
...
def test_ask_response_carries_news():
    ...
    resp = AskResponse(answer="hi", conversation_id=1, news=news)
